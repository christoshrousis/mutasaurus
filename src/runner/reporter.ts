...
export class Reporter {
  formatResults(results: MutasaurusResults): string {
    const { totalMutations, killedMutations, survivedMutations, mutations } = results;
    ...
    if (survivedMutations > 0) {
      output += 'Survived Mutations:\n';
      output += '------------------\n';
      mutations
        .filter((r) => r.status === 'survived')
        .forEach((r) => {
          const { line, column } = this.positionToLineAndColumn(r.original.content, r.start);
  ...
  async generateReport(results: MutasaurusResults, outputPath?: string): Promise<void> {
